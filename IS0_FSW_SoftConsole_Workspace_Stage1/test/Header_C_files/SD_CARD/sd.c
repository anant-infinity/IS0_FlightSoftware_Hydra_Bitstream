...
//	MSS_GPIO_set_output( GPIO_EN_SD1, 1);
	...
	i=0;
	SD_Init(curr_slave);
	i = SD_Write_Data(data, SD_TEST_DATA_SIZE, 0x201, 0);
	i = SD_Read_Data(rx_data, 512, 0x201, 0);
...
//		MSS_SPI_transfer_block(&g_mss_spi1, tmp_buff, 10, 0, 0);
		...
		do {
			MSS_SPI_transfer_block(&g_mss_spi1, tmp_buff, 0, rx_buffer, 1);
			i++;
			for(delay=0; delay < 100; delay++); //Delay needed here for Sandisk
...
uint8_t SD_Set_Highest_Clock(mss_spi_slave_t slave){
	...
	do{
		...
		(
			...
			slave,
			MSS_SPI_MODE0,
			SD_CARD_SINGLE_BLOCK_SIZE,//Globals.SD_Clk_Div,
			8);
